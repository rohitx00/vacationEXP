  ...
  const handleSaveTrip = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/api/trips/save`,
        {
          method: "POST",
          headers: {
              ...
              >
                ...
                <div className="space-y-4 mt-4">
                  {day.activities?.map((act, i) => (
                    <div
                      key={i}
                      className="pl-4 border-l-2 border-secondary/50"
                    >
            ...
            <div className="bg-muted/30 border border-border rounded-xl p-6">
              ...
              <ul className="space-y-2 text-sm">
                <li className="flex justify-between">
                  ...
                  <strong>{itinerary.estimatedBudget?.accommodation}</strong>
                </li>
                <li className="flex justify-between">
                  <span>Food</span>{" "}
                  <strong>{itinerary.estimatedBudget?.food}</strong>
                </li>
                <li className="flex justify-between">
                  <span>Transport</span>{" "}
                  <strong>{itinerary.estimatedBudget?.transport}</strong>
                </li>
                <li className="flex justify-between">
                  <span>Activities</span>{" "}
                  <strong>{itinerary.estimatedBudget?.activities}</strong>
                </li>
                <li className="flex justify-between pt-2 border-t border-border text-base">
                  <span>Total</span>{" "}
