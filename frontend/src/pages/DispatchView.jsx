...
export default function DispatchView() {
  ...
  if (loading) return <div className="min-h-screen p-16 text-center">Loading Dispatch...</div>;
  if (!post) return <div className="min-h-screen p-16 text-center">Dispatch not found.</div>;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="w-full h-[60vh] relative">
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-16 text-white max-w-4xl">
          ...
          <h1 className="text-4xl md:text-6xl font-heading mb-6 leading-tight">{post.title}</h1>
          <p className="text-lg text-white/90">Dispatch by <strong>{post.name}</strong></p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-8 md:p-16 grid grid-cols-1 md:grid-cols-3 gap-12">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-card border border-border p-6 rounded-xl shadow-sm">
            <h3 className="font-heading text-xl text-primary mb-6 border-b border-border pb-2">Traveler Discussion</h3>
            ...
            <form onSubmit={handleComment} className="mt-4 pt-4 border-t border-border">
              <textarea 
                ...
                placeholder={userInfo ? "Add a comment..." : "Log in to comment"}
                rows={3}
                value={commentText}
                onChange={e => setCommentText(e.target.value)}
                disabled={!userInfo}
              />
              <Button type="submit" disabled={!userInfo || !commentText.trim()} className="w-full mt-2">
                Post Comment
              </Button>
            </form>
          </div>
